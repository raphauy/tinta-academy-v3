...
export function DataTableToolbar<TData>({ table }: DataTableToolbarProps<TData>) {
  ...
  return (
    <div className="flex gap-1 dark:text-white items-center">
          ...
          <Input className="max-w-xs" placeholder="dateOfBirth filter..."
              value={(table.getColumn("dateOfBirth")?.getFilterValue() as string) ?? ""}
              onChange={(event) => table.getColumn("dateOfBirth")?.setFilterValue(event.target.value)}                
          />
          ...
          <Input className="max-w-xs" placeholder="zip filter..."
              value={(table.getColumn("zip")?.getFilterValue() as string) ?? ""}
              onChange={(event) => table.getColumn("zip")?.setFilterValue(event.target.value)}                
          />
